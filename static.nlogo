...
turtles-own [
  ...
  personal-preference ;; personal preference for similarity / disimilarity
  preference-from-friends
  friends-attitudes
  similar-friends
  total-friends
  friends-similarity
...
to setup-uniform
  ...
  ask n-of (number / 2) turtles
    [ set color green ]
  ask turtles [set personal-preference %-similar-wanted]
   set-current-plot "%-Similar-Wanted"
   histogram [personal-preference] of turtles
...
to setup-normal
  ...
  ask n-of (number / 2) turtles
    [ set color green ]
  ask turtles [set personal-preference random-normal %-similar-wanted sd-similar-wanted
    if personal-preference < 0 [set personal-preference 0]
    if personal-preference > 100 [set personal-preference 100]]
;;   set-current-plot "%-Similar-Wanted"
;;   histogram [personal-preference] of turtles
end
...
;; each turtle can attach to any other turtle
;; probability of attachment is proportional to the alter's degrees
;; probabilities of attachment sum to one
;; probability of attachment = alter's degrees / total links in the network
  set total-links count links
  ask one-of turtles [create-link-with one-of turtles]
  ask turtles [ 
    ask one-of links [set partner one-of both-ends ]
    if partner != self [create-link-with partner]]
  ask links [hide-link]
end

to show-social-network
  ask links [show-link]
  repeat 10
  [
...
to update-turtles
  ask turtles [
    ...
    set similar-friends count (link-neighbors) with [color = [color] of myself]
    set total-friends count (link-neighbors)
    if total-friends != 0 [
      set friends-similarity similar-friends / total-friends * 100;; how similar my friends are to me
      set friends-attitudes (sum [personal-preference] of link-neighbors) / total-friends;; what my friends' preference is, regardless of my friends' color
      set preference-from-friends friends-attitudes-makeup * friends-attitudes / 100 + (100 - friends-attitudes-makeup) * friends-similarity / 100 
      ;; weighted sum of friends' similarity and composition (makeup) -- weight is friends-attitudes-makeup
      set preference personal-vs-friends * personal-preference / 100 + (100 - personal-vs-friends) * preference-from-friends / 100]
      ;; final preference, weighted sum of personal-preference (with which I start the round), and friends' preference (calculated above) - weight is personal-vs-friends
      
    if total-friends = 0 [
      set preference personal-preference]
    set happy similar-nearby >= ( preference * total-nearby / 100 )
  ]
  ask turtles [set personal-preference preference]
  set-current-plot "%-Similar-Wanted"
  plot-pen-reset
  histogram [preference] of turtles
...
501
Personal Preference
mean [personal-preference] of turtles
2
1
11
...
@#$#@#$#@
<experiments>
  <experiment name="Testrun" repetitions="5" runMetricsEveryStep="true">
    <setup>setup-uniform
create-scale-free</setup>
    <go>go</go>
    <timeLimit steps="200"/>
    <metric>percent-similar</metric>
    <metric>percent-unhappy</metric>
    <steppedValueSet variable="friends-attitudes-makeup" first="0" step="5" last="100"/>
    <steppedValueSet variable="personal-vs-friends" first="0" step="5" last="100"/>
    ...
    <enumeratedValueSet variable="sd-similar-wanted">
      <value value="15"/>
    </enumeratedValueSet>
